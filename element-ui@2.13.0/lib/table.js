...
/* harmony default export */ var current = ({
  data: function data() {
    return {
      states: {
        // 不可??的，?置 currentRowKey ?，data 不一定存在，也?无法算出正?的 currentRow
        // 把???存一下，当用?点?修改 currentRow ?，把??重置? null
        _currentRowKey: null,
        currentRow: null
      }
    };
...
/* harmony default export */ var tree = ({
  data: function data() {
    return {
      states: {
        // defaultExpandAll 存在于 expand.js 中，?里不重?添加
        // 在展?行中，expandRowKeys 会被?化成 expandRows，expandRowKeys ?个属性只是??了 TreeTable 行的展?
        // TODO: 拆分?独立的 TreeTable，?一用法
        expandRowKeys: [],
        treeData: {},
        indent: 16,
        lazy: false,
  ...
  computed: {
    // 嵌入型的数据，watch 无法是??到?化 https://github.com/ElemeFE/element/issues/14998
    // TODO: 使用 computed 解决???，是否会造成性能??？
  ...
  methods: {
    normalize: function normalize(data) {
      ...
      Object(util["n" /* walkTreeNode */])(data, function (parent, children, level) {
        ...
        if (Array.isArray(children)) {
          ...
          // 当 children 不存在且 lazy ? true，??点即??加?的?点
          res[parentId] = {
            children: [],
            lazy: true,
            level: level
    ...
    updateTreeData: function updateTreeData() {
      ...
      if (keys.length) {
        ...
        // 合并 expanded 与 display，?保数据刷新后，状?不?
        keys.forEach(function (key) {
        ...
        if (lazy && lazyKeys.length && rootLazyRowKeys.length) {
          lazyKeys.forEach(function (key) {
            ...
            if (rootLazyRowKeys.indexOf(key) !== -1) {
              // ?加?的 root ?点，更新一下原有的数据，原来的 children 一定是空数?
...
/* harmony default export */ var watcher = (external_vue_default.a.extend({
  ...
  methods: {
    ...
    // ?配?，expand-row-keys 在 Expand 与 TreeTable 中都有使用
...
watcher.prototype.mutations = {
  setData: function setData(states, data) {
    ...
    this.execQuery();
    // 数据?化，更新部分数据。
    // 没有使用 computed，而是手?更新部分数据 https://github.com/vuejs/vue/issues/6660#issuecomment-331417140
  ...
  changeSortCondition: function changeSortCondition(states, options) {
    // 修? pr https://github.com/ElemeFE/element/pull/15012 ?致的 bug
...
/* harmony default export */ var table_body = ({
  ...
  methods: {
    ...
    handleCellMouseEnter: function handleCellMouseEnter(event, row) {
      ...
      // 判断是否text-overflow, 如果是就?示tooltip
    ...
    wrappedRowRender: function wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        assertRowKey();
        // TreeTable ?，rowKey 必?由用??定，不使用 getKeyOfRow ?算
        // 在?用 rowRender 函数?，仍然会?算 rowKey，不太好的操作
        ...
        // ?染嵌套数据
        if (cur) {
          // currentRow ??的是 index，所以?需主??加 TreeTable 的 index
          var i = 0;
          var traverse = function traverse(children, parent) {
            if (!(children && children.length && parent)) return;
            children.forEach(function (node) {
              // 父?点的 display 状?影?子?点的?示状?
              ...
              // ?于当前?点，分成有无子?点??情况。
              // 如果包含子?点的，?置 expanded 属性。
              // ?于它子?点的 display 属性由它本身的 expanded 与 display 共同决定。
              if (cur) {
